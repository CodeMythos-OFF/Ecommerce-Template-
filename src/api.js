...
export const updateEmailTemplate = async (key, template) => {
  const response = await fetchWithRetry(`${API_URL}/email/templates/${encodeURIComponent(key)}`, {
    method: 'PUT',
    headers: getAuthHeaders({ 'Content-Type': 'application/json' }),
...
export const sendTestEmail = async (to = '') => {
  const response = await fetchWithRetry(`${API_URL}/email/test`, {
    method: 'POST',
    headers: getAuthHeaders({ 'Content-Type': 'application/json' }),
